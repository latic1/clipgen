...
# This code sets up a Modal image with specific configurations and dependencies.
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    print(
        f"Clip {clip_index} vertical video creation time: {cvv_end_time - cvv_start_time:.2f} seconds")# create_subtitles_with_ffmpeg(transcript_segments, start_time,
    ...
    s3_client.upload_file(
        subtitle_output_path, "clipgen-temp-storage", output_s3_key)
...
class AiPodcastClipper:
    ...
    def process_video(self, request:ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client('s3')
        s3_client.download_file("clipgen-temp-storage", s3_key, str(video_path))
